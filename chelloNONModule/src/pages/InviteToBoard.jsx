...
export default function  InviteToBoard(){
    ...
    const generateLink3 = async()=>{
        if(emailList.includes(inviteEmail) ||listInvited.includes(inviteEmail)){
            ...
            const docRef = await addDoc(coll, {
                ...
                const ref = doc(db,"workspaces",workspaceID, "boards", boardID);
                await updateDoc(ref,{
    ...
    const generateLink1 = async() =>{
        const brRef = doc(db, "workspaces", workspaceID, "boards", boardID)
        const exec = await getDoc(brRef)
        setListInvited(exec.data().listInvited)
        setMemberList(exec.data().boardMember)
    }
